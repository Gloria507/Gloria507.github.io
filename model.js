...
//远程异步读取三个大型单词文本
    fetch('cet/text1.txt') //读取cet/cet1.txt
    .then(resp => resp.text())
    .then(txt => {
        Model.CET6 = [] ;
        createCET6(txt) ;
    ...
    setTimeout(function(){ //读取cet/cet2.txt
        fetch('cet/text2.txt') //读取cet/cet2.txt
        .then(resp => resp.text())
        .then(txt => {
         createCET6(txt) ;
        UI.log('系统成功读取了'+ Model.CET6.length +'个单词！') ;
 ...
 let UI = {} ; //UI用于表达用户界面，以及改变用户界面上的的内容
  ...
  UI.footerLog = function(s){
    ...
    setTimeout(() => {
        select('footer').textContent = "江西科技师范大学 吴忆东 2024--2025" ;
    }, 3*1000);
  };
  ...
  UI.userStatus = function(){
    ...
    setTimeout(()=>{
        select('p#title').textContent = 'Relax-轻轻松松背单词';
    },1000*10)
  } ;
